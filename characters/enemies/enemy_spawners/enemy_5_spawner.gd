...
@export var max_pitch_scale = 0.7
...
var enemy_types = [
	{"scene": bird_enemy_melee_scene, "weight": bird_enemy_melee_spawn_weight},  # 25% chance to spawn
	{"scene": bird_enemy_ranged_scene, "weight": bird_enemy_ranged_spawn_weight},   # 45% chance to spawn
	{"scene": reptile_enemy_melee_scene, "weight": reptile_enemy_meleed_spawn_weight}   # 30% chance to spawn
]

@onready var enemy_5_spawn_sound = $SpawnSound/Enemy5SpawnSound
...
func _spawn_enemy():
	...
	for enemy_type in enemy_types:
		...
		if random_pick < current_sum:
			...
			$Enemy5SpawnEffect/SpawnFlames.restart() # Spawn effects
			$Enemy5SpawnEffect/SpawnSmoke.restart() # Spawn effects
...
func play_spawn_sound():
	...
	enemy_5_spawn_sound.pitch_scale = random_pitch
	enemy_5_spawn_sound.play()
