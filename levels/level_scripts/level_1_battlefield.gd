...
signal level_1_time_survived
signal level_1_time_not_survived
...
func _ready():
	...
	general_ambience_level_1.play()
	
	await get_tree().create_timer(10).timeout
	level_timer.visible = true
	level_timer.start_timer()
	
	battle_ambience_level_1.play()
	await get_tree().create_timer(4).timeout
	general_ambience_level_1.queue_free()
	
	enemy_spawner_manager.manager_start_spawning()

func _process(delta):
	if level_timer.get_time() > 120.0 and not weapon_unlocked:
		level_1_time_survived.emit()
	...
	if player.dead:
		level_1_time_not_survived.emit()
