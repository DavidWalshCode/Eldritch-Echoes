...
signal level_3_time_survived
signal level_3_time_not_survived
...
func _process(delta):
	if level_timer.get_time() > 180.0 and not weapon_unlocked:
		level_3_time_survived.emit()
	...
	if player.dead:
		level_3_time_not_survived.emit()
