...
# Used by scene manager to display transitions and loading progress
...
# Hides progress bar on startup, we'll reveal it later if loading has taken long enough that it's worth showing. 
# The alternative is that when something loads quickly it flashes on screen briefly, and I don't like that.
func _ready() -> void:
